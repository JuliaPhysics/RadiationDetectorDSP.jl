...
# Kernels on the KernelAbstractions CPU backend index GPU-style arrays
# element-wise, possibly on spawned tasks, so the task-local
# GPUArraysCore.@allowscalar is not sufficient:
function with_allowscalar(f)
